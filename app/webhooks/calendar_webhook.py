...
# Lägg till fler gym-slugs här om ni lägger till fler gym i framtiden
...
def _get_calendar_for_gym(gym_slug: str) -> Optional[GoogleCalendar]:
    """
    Skapar en GoogleCalendar-instans för ett specifikt gym.
    Returnerar None om kalendern inte är konfigurerad.
    """
    try:
        ...
        print(f"[CALENDAR WEBHOOK] Could not create calendar for gym {gym_slug}: {e}")
        return None
...
async def process_calendar_changes():
    ...
    Loopar igenom alla konfigurerade gym och hämtar events från respektive kalender.
    """
    db = SessionLocal()
    try:
        ...
        total_matched_leads = 0

        # Loopa igenom alla gym och hämta events från respektive kalender
        for gym_slug in ALL_GYM_SLUGS:
            calendar = _get_calendar_for_gym(gym_slug)
            if not calendar:
                print(f"[CALENDAR WEBHOOK] Skipping gym '{gym_slug}' - calendar not configured or not available")
                continue

            print(f"[CALENDAR WEBHOOK] Fetching events for gym '{gym_slug}'...")
            events = calendar.get_recent_events(start_time=start_time, end_time=end_time)
            print(f"[CALENDAR WEBHOOK] Found {len(events)} events for gym '{gym_slug}'")
            ...
            for event in events:
                ...
                print(f"[CALENDAR WEBHOOK] Processing new event: {event_id} (gym: {gym_slug})")
            ...
            total_new_bookings += new_bookings_count
            total_matched_leads += matched_leads_count
            print(f"[CALENDAR WEBHOOK] Gym '{gym_slug}': {new_bookings_count} new bookings, {matched_leads_count} leads matched")

        db.commit()
        print(f"[CALENDAR WEBHOOK] Total: {total_new_bookings} new bookings, {total_matched_leads} leads matched")
...
async def _process_matched_booking(
    ...
    Kräver nu gym_slug för att veta vilken kalender bokningen hör till.
    """
    try:
        # Parse event time
        event_start = event.get("start", {}).get("dateTime")
