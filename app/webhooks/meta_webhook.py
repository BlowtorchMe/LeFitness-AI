...
def _get_gym_slug_for_lead(db: Session, lead) -> Optional[str]:
    """
    Hämtar gym_slug för en lead baserat på selected_gym_id.
    Returnerar None om inget gym är valt.
    """
    if not lead or not lead.selected_gym_id:
        return None
    gym = db.query(Gym).filter(Gym.id == lead.selected_gym_id).first()
    return gym.slug if gym else None
...
async def handle_messaging_event(event: Dict, channel: ConversationChannel):
    ...
    if ai_response.get("should_proceed"):
        flow_result = await flow.handle_state_transition(
            ...
            # Skickar gym_slug för att ConversationFlow ska kunna hämta rätt kalender
            gym_slug=gym_slug
        )
    ...
    if is_calendar_request and current_state in ["profile_complete", "recommending_booking", "collecting_booking_details"]:
        ...
        # Provide appointment schedule link directly (med gym_slug)
        calendar_link = flow._get_calendar_link(datetime.now(), gym_slug)
...
async def handle_profile_gathering(
    ...
    conversation_service = ConversationService(lead_service.db)
    # Hämta gym_slug för denna lead
    gym_slug = _get_gym_slug_for_lead(lead_service.db, lead)
    ...
    if status == "waiting_for_name":
        ...
        if status == "complete":
            ...
            flow = ConversationFlow()
            booking_service = BookingService(lead_service.db)
            await flow._recommend_booking(lead, sender_id, lead_service, gym_slug)
...
async def handle_postback(sender_id: str, payload: str, channel: ConversationChannel):
    ...
    current_state = lead.conversation_state or "welcome"
    # Hämta gym_slug för denna lead (behövs för alla booking/kalender-anrop)
    gym_slug = _get_gym_slug_for_lead(db, lead)
    ...
    if payload == "BOOK_NOW" or payload.startswith("BOOK_"):
        ...
        if payload in ["BOOK_TOMORROW", "BOOK_THIS_WEEK", "BOOK_NEXT_WEEK"]:
            ...
            await flow._collect_booking_details(lead, sender_id, booking_service, gym_slug)
            ...
            await flow._create_booking(lead, sender_id, booking_service, lead_service, gym_slug)
        elif payload == "BOOK_MORE_TIMES":
            tomorrow = datetime.now() + timedelta(days=1)
            # get_available_slots kräver nu gym_slug
            if gym_slug:
                slots = booking_service.get_available_slots(tomorrow, gym_slug=gym_slug)
            else:
                slots = []
            ...
            if len(slots) > 3:
                ...
                time_buttons.append({
                    ...
                    "url": flow._get_calendar_link(tomorrow, gym_slug)
                })
        ...
        if current_state == "recommending_booking":
            flow = ConversationFlow()
            await flow._continue_recommending(lead, sender_id, gym_slug)
