...
class ChatHandler:
    ...
    async def process_message(
        ...
        if analysis.fast_path_response:
            ...
            if intent == "overview":
                self.logger.info(
                    "ai_timing_direct_overview intent=%.3f total=%.3f",
                    t1 - t0,
                    t_direct - t0,
                )
            else:
                ...
                self.logger.info(
                    "ai_timing_direct_faq intent=%.3f faq=%.3f total=%.3f score=%.3f",
                    t1 - t0,
                    t2 - t1,
                    t_direct - t0,
                    score,
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=messages,
                temperature=0.4,
                max_tokens=self._max_tokens_for_intent(intent, current_state),
            )
            t3 = perf_counter()
