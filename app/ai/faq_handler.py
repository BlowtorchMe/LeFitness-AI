...
def _warm_components_sync() -> bool:
    ...
    try:
        ...
        print(f"FAQ warmup took {(perf_counter() - t0)*1000:.1f}ms")
        return True
    except Exception:
...
def _retrieve_match_sync(question: str) -> Optional[FAQMatch]:
    ...
    if direct_match:
        print("FAQ direct keyword match (no OpenAI call)")
        print(f"FAQ total took {(perf_counter() - total_start)*1000:.1f}ms")
        return direct_match
    _ensure_pg_conn_str()
    if not settings.openai_api_key:
        return None
    try:
        ...
        t2 = perf_counter()
        print(f"FAQ embedder.run took {(t2-t1)*1000:.1f}ms")
        embedding = out.get("embedding")
        if not embedding:
            print("FAQ no embedding returned")
            return None
        result = retriever.run(query_embedding=embedding, top_k=1)
        t3 = perf_counter()
        print(f"FAQ retriever.run took {(t3-t2)*1000:.1f}ms")
        docs = result.get("documents") or []
        print(f"FAQ docs found: {len(docs)}")
        if not docs:
            print(f"FAQ total took {(perf_counter() - total_start)*1000:.1f}ms")
            return None
        doc = docs[0]
        ...
        import os as _os
        min_score = float(_os.environ.get("FAQ_MIN_SCORE", "0.6"))
        print(f"FAQ top score: {score:.4f}  min_score: {min_score}")
        if score < min_score:
            print("FAQ score below threshold")
            print(f"FAQ total took {(perf_counter() - total_start)*1000:.1f}ms")
            return None
        if not answer:
            print("FAQ match had no answer")
            return None
        print(f"FAQ total took {(perf_counter() - total_start)*1000:.1f}ms")
